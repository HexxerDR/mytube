...
class Video(models.Model):
    videoemb = models.FileField(max_length=150, upload_to=uploadPath, null=False, validators=[FileExtensionValidator(allowed_extensions=["mov","mp4","avi","wmv"])])
...
def vidThumbnailCreate(sender, instance, **kwargs):
    if instance.created == 0:
        if instance._state.adding is False:
            ...
            os.rename("videosuploads/{}/{}".format(slugify(instance.author.verToken), (instance.vidID)), "videosuploads/{}/{}".format(slugify(instance.author.verToken), (acPath)))
            ...
            vidpath = "videosuploads/{}/{}/{}".format(instance.author.verToken, acPath, thmDes)
            thmpath = "videosuploads/{}/{}/{}.png".format(instance.author.verToken, acPath, thmBase)
            process = subprocess.run(["ffmpeg", "-i", vidpath, "-ss", "00:00:05", "-vf", "thumbnail, scale=1920:1080", "-frames:v", "1", thmpath])
            instance.created += 1
            instance.thumbnail = thmpath
            instance.save()
